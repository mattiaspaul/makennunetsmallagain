...
class GroupView(torch.nn.Module):
    ...
    def __init__(self,dim_in,dim_out,stride,groups=4):
        ...
        self.groups = groups
        self.stride = stride
        self.conv = nn.Sequential(nn.Flatten(0,1),nn.Unflatten(0,(-1,dim_in//groups)),\
                                  nn.Conv3d(dim_in//groups, dim_out*2//groups, 3,padding=1,bias=True),\
                                  nn.Flatten(0,1),nn.Unflatten(0,(-1,dim_out*2)))

        self.norm = nn.BatchNorm3d(dim_out*2)
        self.cout = nn.Conv3d(dim_out*2,dim_out,1)
        self.relu = nn.LeakyReLU(negative_slope=1e-2,inplace=True)
    def forward(self, x: Tensor) -> Tensor:
        x = self.conv(x)
        x = self.relu(self.cout(self.norm(x)))
...
class nnUNetTrainerV2_noMirroring_groupview_bn(nnUNetTrainer):
    """
    ...
    def initialize_network(self):
        ...
        for name, module in self.network.named_modules():
            if isinstance(module,ConvDropoutNormNonlin):
                ...
                if(before.conv.out_channels>=160):
                    dim1 = before.conv.in_channels
                    dim2 = before.conv.out_channels
                    block1 = GroupView(dim1,dim2,before.conv.stride,4)
